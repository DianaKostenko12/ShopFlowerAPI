namespace BLL.Services.Colors
{
    public static class BaseColorNormalizer
    {
        ...
        public static List<string> GetRelatedColors(string baseColor)
        {
            var normalizedBaseColor = Normalize(baseColor);
            if (string.IsNullOrWhiteSpace(normalizedBaseColor))
            {
                return [];
            }

            return AliasToBaseColor
                .Where(pair => pair.Value.Equals(normalizedBaseColor, StringComparison.OrdinalIgnoreCase))
                .Select(pair => pair.Key)
                .Append(normalizedBaseColor)
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToList();
        }
